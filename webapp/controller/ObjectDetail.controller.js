sap.ui.define([
	...
	return Base.extend("com.bosch.rb1m.tm.tmtnadu.controller.ObjectDetail", {
		...
		onInit: function () {
                                ...
                                if(_response1.headers["sap-message"])
                                    {
                                       ...
                                       if(c.severity=="success"){
                                        if(c.message){
											...
											var aItemData = that.getModel("objectModel").getProperty("/dynamicForm");
											var bIsMode = that.getModel("objectModel").getProperty("/ModeChange");	;
											...
											that.oEditAction.setVisible(true);			
											that.getOwnerComponent().getEventBus().publish("MainViewTable", "ItemChange", {			
												aItem: aItemData,
												bIsMode: bIsMode
											});
											if( that.getModel("objectModel").getProperty("/ModeChange") == "D"){
												that.getModel("objectModel").setProperty("/dynamicForm",[]);
			...
			this.oMultiInput1 = this.getView().byId("_IDMulti_UNAME_2");
			this.oMultiInput1.setValue("");
			this.oMultiInput1.removeAllTokens();
			this.oMultiInput1.addValidator(function(args){     
				//debugger;        
				var text = args.text;
				var text1 = args.text;
		...
		onRouteMatched: function () {
			...
			var oData = {};		
			if(this.getOwnerComponent().getModel("objectDetail")){
				var odataObjDetail = this.getOwnerComponent().getModel("objectDetail").getProperty("/objectDetail");
			...
			if (oData) {				
				if(oData.mode == "C")
				{
					...
					//pageTitle
					this.getModel("objectModel").setProperty("/pageTitle",oData.pageTitle);	
		...
		fnGetDataHeaderItem: function(oData)
		{
			var objData = oData;	
			let arrFieldsSet = [];
			arrFieldsSet.push(objData.rows);
			this.getModel("objectModel").setProperty("/dynamicForm",arrFieldsSet);			
			if(arrFieldsSet[0]){
				if(arrFieldsSet[0]["Uname"]){
					this.oMultiInput1 = this.getView().byId("_IDMulti_UNAME_2");
				    this.oMultiInput1.setTokens([
						new Token({text: arrFieldsSet[0]["Uname"], key: arrFieldsSet[0]["Uname"]})
					]);
					if(arrFieldsSet[0]["Uname"].length < 1){
						this.oMultiInput1.removeAllTokens();
					}
				}
				else{
				...
				//OUTPUT_TYPE
				if(arrFieldsSet[0]["OutputType"]){
					this.getModel("objectModel").setProperty("/seloutputtype",arrFieldsSet[0]["OutputType"]);	
					//oDataForm[0]["OutputType"] 
				}
			}
		...
		fnDeleteData: function(){
			...
			_payload_deep_rt.push(oDataForm[0]);
		...
		onEdit: function()
		{
			...
			var aTokensUname = this.oMultiInput1.getTokens();
			
			if(oDataForm[0])
			{
				if(aTokensUname[0]){
					oDataForm[0]["Uname"] = aTokensUname[0].getKey();
				}		
			}
			if(oDataForm)
			{                  
		...
		//	debugger;		
			...
			if(oDataForm[0]["OutputType"].length < 1)
			{
				chkKeyValue = false;
				let _message = this.getResourceBundle().getText("dialog.error.CheckMandatory.Invalid") + " " + this.getResourceBundle().getText("output_type_txt");
				sap.m.MessageBox.show(_message, {
					icon: sap.m.MessageBox.Icon.ERROR,
					title: "Error"
					});
			}	
			if(oDataForm[0]["Uname"].length < 1)
				{
					chkKeyValue = false;
					let _message = this.getResourceBundle().getText("dialog.error.CheckMandatory.Invalid") + " " + this.getResourceBundle().getText("uname_txt");
					sap.m.MessageBox.show(_message, {
						icon: sap.m.MessageBox.Icon.ERROR,
						title: "Error"
						});
			...
			let oModeChange = this.getModel("objectModel").getProperty("/ModeChange");
			_payload_deep_rt.push(oDataForm[0]);
			if(_payload_deep_rt.length > 0)
			{
				this.setBusy(true);     
				this.getModel("objectModel").setProperty("/batchCompleted", false);
				this.batchObjectActive = true;    
				this.getService().sendMultiAction("/xRB1MxTM_I_TNADU",_payload_deep_rt,oModeChange).then(
                    function (aData) {
                       //debugger;
                        //this.setBusy(false);
                        //this._fnHandleSuccessExe();
